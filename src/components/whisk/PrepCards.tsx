...
import { type PrepStatus, prepItems } from "@/lib/whiskData";
    ...
    <div className="grid gap-4 md:grid-cols-3">
      {prepItems.map((item) => {
        ...
        return (
          <div
            key={item.id}
            className={`group rounded-2xl border border-border bg-card p-5 shadow-elev-sm transition-all hover:shadow-elev-md ${cfg.ring}`}
          >
            <div className="flex items-start justify-between gap-2">
              <div
                className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${cfg.chip}`}
              >
